...
class CalendarEventSeeder extends Seeder
{
    public function run()
    {
        ...
        for ($i = 0; $i < 10; $i++) {
            $start = Carbon::parse($faker->dateTimeBetween('now', '+6 months'));
            $end = (clone $start)->addHours($faker->numberBetween(1, 4));

            $location =
                $faker->randomElement(['Conference Center', 'Tech Hub', 'Auditorium', 'Convention Hall'])
                . ', ' . $faker->city() . ', ' . $faker->stateAbbr() . ' ' . $faker->postcode(); // Full address

            $overview = '<p>' . $faker->paragraph(2) . '</p>' .
                '<p><b>' . $faker->sentence() . '</b></p>' .
                '<ul>' .
            ...
            CalendarEvent::create([
                ...
                'image'       => 'https://picsum.photos/590/590?random=' . $faker->unique()->numberBetween(1, 1000),
